...
def initialize_database():
    ...
    if not db.table("devices").all():
        db.table("devices").insert_multiple([
            {"device_id": "Device1", "managed_by_user_id": "one@mci.edu", "is_active": True},
            {"device_id": "Device2", "managed_by_user_id": "two@mci.edu", "is_active": True}
        ])
...
def reservation_management():
    ...
    selected_device_name = st.selectbox("Gerät auswählen", options=list(device_options.keys()), key="reservation_device_select")
    selected_device = device_options[selected_device_name]
...
# Wartungsverwaltung
    ...
    selected_device_name = st.selectbox("Gerät auswählen", options=list(device_options.keys()), key="maintenance_device_select")
    selected_device = device_options[selected_device_name]
